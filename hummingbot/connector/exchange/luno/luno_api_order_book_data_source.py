...
class LunoAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_new_order_book(self, trading_pair: str) -> OrderBook:
        async with aiohttp.ClientSession() as client:
            ...
            snapshot_msg: OrderBookMessage = LunoOrderBook.snapshot_message_from_exchange(
                snapshot,
                snapshot_timestamp,
    ...
    def get_mid_price(trading_pair: str) -> Optional[Decimal]:
        from hummingbot.connector.exchange.luno.luno_utils import convert_to_exchange_trading_pair
    ...
    async def fetch_trading_pairs() -> List[str]:
        ...
        #             if response.status == 200:
        #                 from hummingbot.connector.exchange.luno.luno_utils import convert_from_exchange_trading_pair
    ...
    async def listen_for_trades(self, ev_loop: asyncio.BaseEventLoop, output: asyncio.Queue):
        ...
        while True:
            ...
            #             for trade in trades:
            #                 trade_msg: OrderBookMessage = LunoOrderBook.trade_message_from_exchange(trade)
            ...
            #                         exc_info=True)
            self.logger().info("faking Luno trade")
            ...
            update = {
                ...
                "tradeUpdate": {
                    ...
                    "base": Decimal(random.random()),
                    "counter": Decimal(random.random()),
                    "timestamp": time.time_ns() / 1e6,
                    "takerOrderId": random.randint(1e6, 1e9),
                    "makerOrderId": random.randint(1e6, 1e9),
                }
            }

            trade_msg: OrderBookMessage = LunoOrderBook.trade_message_from_exchange("ETHXBT", update)
            output.put_nowait(trade_msg)
    ...
    async def listen_for_order_book_diffs(self, ev_loop: asyncio.BaseEventLoop, output: asyncio.Queue):
        ...
        while True:
            ...
            #                 order_book_message: OrderBookMessage = LunoOrderBook.snapshot_ws_message_from_exchange(
            #                     msg_dict, time.time())
            #             else:
            #                 order_book_message: OrderBookMessage = LunoOrderBook.diff_message_from_exchange(
            #                     msg_dict, time.time())
            ...
            #     self.logger().error("Unexpected error with WebSocket connection. Retrying after 30 seconds...",
            #                         exc_info=True)
            self.logger().info("faking Luno order book diff")
            ...
            order_book_message: OrderBookMessage = LunoOrderBook.diff_message_from_exchange("ETHXBT", update)
            output.put_nowait(order_book_message)
            await asyncio.sleep(random.randint(3, 10))
    ...
    async def listen_for_order_book_snapshots(self, ev_loop: asyncio.BaseEventLoop, output: asyncio.Queue):
        while True:
            ...
            # except Exception:
            #     self.logger().error("Unexpected error. ", exc_info=True)
            self.logger().info("listening for Luno order book snapshots")
            await asyncio.sleep(random.randint(3, 10))
