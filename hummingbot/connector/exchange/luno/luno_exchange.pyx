...
from hummingbot.connector.exchange.luno.luno_order_book_tracker import LunoOrderBookTracker
from hummingbot.connector.exchange.luno.luno_user_stream_tracker import LunoUserStreamTracker
from hummingbot.connector.exchange.luno.luno_in_flight_order import LunoInFlightOrder
...
cdef class LunoExchangeTransactionTracker(TransactionTracker):
    cdef:
        LunoExchange _owner

    def __init__(self, owner: LunoExchange):
        super().__init__()
        self._owner = owner
...
cdef class LunoExchange(ExchangeBase):
    ...
    def __init__(self,
        ...
        self._order_book_tracker = LunoOrderBookTracker(trading_pairs=trading_pairs)
        self._kraken_auth = LunoAuth(luno_api_key, luno_api_secret)
        self._user_stream_tracker = LunoUserStreamTracker(kraken_auth=self._kraken_auth)
        self._ev_loop = asyncio.get_event_loop()
        self._poll_notifier = asyncio.Event()
        self._last_timestamp = 0
        self._poll_interval = poll_interval
        self._in_flight_orders = {}  # Dict[client_order_id:str, LunoInFlightOrder]
        self._order_not_found_records = {}  # Dict[client_order_id:str, count:int]
        self._tx_tracker = LunoExchangeTransactionTracker(self)
    ...
    @property
    def in_flight_orders(self) -> Dict[str, LunoInFlightOrder]:
    ...
    def restore_tracking_states(self, saved_states: Dict[str, Any]):
        in_flight_orders: Dict[str, LunoInFlightOrder] = {}
        for key, value in saved_states.items():
            in_flight_orders[key] = LunoInFlightOrder.from_json(value)
    ...
    async def get_active_exchange_markets(self) -> pd.DataFrame:
        return await LunoAPIOrderBookDataSource.get_active_exchange_markets()
    ...
    async def _update_balances(self):
        ...
        balances = {"ETH": "10.987", "XBT": "5.432"} #await self._api_request_with_retry("POST", BALANCE_URI, is_auth_required=True)
        open_orders = {"open": {}} #await self._api_request_with_retry("POST", OPEN_ORDERS_URI, is_auth_required=True)
        ...
        self.logger().info(f"updated available balances {self._account_available_balances}")
    ...
    cdef object c_get_fee(self,
        ...
        return estimate_fee("luno", is_maker)
    ...
    async def _update_trading_rules(self):
        ...
        self._trading_rules["ETH-BTC"] = TradingRule(
                        "ETH-BTC",
    ...
    async def _user_stream_event_listener(self):
        ...
        async for msg in self._iter_user_event_queue():
            self.logger().info(f"got user message: {msg}")
    ...
    async def place_order(self,
        ...
        #                                           is_auth_required=True)
        return {
            "txid": [str(random.randint(1e6, 1e9))]
        }
    ...
    async def execute_buy(self,
        ...
        try:
            ...
            self._user_stream_tracker.data_source.put_user_order(exchange_order_id)
        except asyncio.CancelledError:
            raise
    ...
    async def execute_cancel(self, trading_pair: str, order_id: str):
        self.logger().info(f"execute_cancel: {order_id}")
    ...
    cdef c_start_tracking_order(self,
                                ...
                                object order_type,
                                int userref):
        self._in_flight_orders[order_id] = LunoInFlightOrder(
    ...
    def buy(self, trading_pair: str, amount: Decimal, order_type=OrderType.MARKET,
        ...
        self.logger().info(f"buy: {amount} ({trading_pair} - {order_type})")
        return self.c_buy(trading_pair, amount, order_type, price, kwargs)

    def sell(self, trading_pair: str, amount: Decimal, order_type=OrderType.MARKET,
             price: Decimal = s_decimal_NaN, **kwargs) -> str:
        self.logger().info(f"sell: {amount} ({trading_pair} - {order_type})")
